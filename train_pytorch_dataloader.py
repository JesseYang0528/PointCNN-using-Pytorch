...
import data_augmentation as d_utils
...
class UOY_dataset(Dataset):
    ...
    def __getitem__(self, i):
        if self.transform is not None:
            return self.data[i], self.transform(self.data[i]), self.labels[i]
        return self.data[i], self.data[i], self.labels[i]
...
train_dataset = UOY_dataset(current_data, current_label, transform = transforms)
train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=4)

test_dataset = UOY_dataset(test_data, test_label)
...
for epoch in range(1, NUM_EPOCHS+1):
    ...
    for batch_idx, (data, tf_data, label) in enumerate(train_loader):
        losses = []
        ...
        out = model((data.cuda(), tf_data.cuda()))
        ...
        train_acc.append(correct.item() / float(len(data)))
    ...
    for batch_idx, (data, tf_data, label) in enumerate(test_loader):
        out = model((data.cuda(), data.cuda()))
        pred_label = out.data.max(1)[1]
        # pred_label = torch.max(test_out.data, 1)[1]
        correct = pred_label.eq(label.cuda()).cpu().sum()
        acc = correct.item() / float(len(data))
        test_acc.append(acc)
        torch.cuda.empty_cache()
