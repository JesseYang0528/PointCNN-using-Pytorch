...
parser.add_argument('--batch_size', type=int, default=256, help='Batch Size during training [default: 32]')
...
suffix = '_' + str(LEARNING_RATE) + '_' + str(BATCH_SIZE) + '_' + str(NUM_EPOCHS)
...
for epoch in range(1, NUM_EPOCHS+1):
    ...
    for fn in range(len(TRAIN_FILES)):
        ...
        if epoch > 1:
            ...
            if LEARNING_RATE > LEARNING_RATE_MIN:
                ...
                with open(DATA_SAVE_DIR + prefix + suffix + "_lr.txt", "a+") as f:
                    f.write('Epoch: ' + str(epoch) + ' Train Files: ' + str(fn) + ' Lr: ' + str(LEARNING_RATE) + ' DECAY_RATE: ' + str(DECAY_RATE) + ' global_step: ' + str(global_step) + 'DECAY_STEP: ' + str(DECAY_STEP) + "\n")
                    f.close()
        ...
        with open(DATA_SAVE_DIR + prefix + suffix + "_losses.txt", "a+") as f:
            f.write('Epoch: ' + str(epoch) + ' Train Files: ' + str(fn) + ' loss: ' + str(loss.item()) + "\n")
            f.close()
    ...
    ax.set_ylabel('Loss')
    ax.legend()
    fig.savefig(DATA_SAVE_DIR + prefix + suffix + "_loss.png")
    plt.close(fig)
